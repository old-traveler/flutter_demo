...
class Solution {
    public TreeNode sortedArrayToBST(int[] nums) {
        ...
        return _sortedArrayToBST(0, nums.length, nums);
    }

    private TreeNode _sortedArrayToBST(int start, int end,int[] nums){
        if(start >= end) return null;
        int mid = (start + end) >> 1;
        TreeNode node = new TreeNode(nums[mid]);
        node.left = _sortedArrayToBST(start, mid, nums);
        node.right = _sortedArrayToBST(mid + 1, end, nums);
        return node;
    }
}
```
